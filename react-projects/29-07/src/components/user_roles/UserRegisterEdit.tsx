...
interface UserRegisterProps {
    ...
    isUserDataUpdate : any;
    setIsUserDataUpdated : any;
    isTokenExits : any;
  }
...
type SelectedOption = any;
 ...
 const UserRegister : React.FC<UserRegisterProps> = ({ edituserShow, setEdituserShow, apiUrl, editUserId, setSucessModelShow, setSucessModelMessage ,setIsUserDataUpdated,isUserDataUpdate, loginUserRoleId, loginUserAllData,isTokenExits})  => {
  ...
  const handleChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    const handleSubmit  = async (e: FormEvent<HTMLFormElement>) => {
        ...
        if (!result.success) {
          ...
          if (isTokenExits) {
              try {
                let response =  await axios.put(`${apiUrl}/user/update/${editUserId}`, {
                ...
                setSucessModelMessage("User information’s has been edited successfully");
                setTimeout(() => {
...
// for Edit 
 ...
 const getSingleUser  = async () => {
  if (isTokenExits) {
    try {
      let get_all_User =  await axios.get(`${apiUrl}/user/show/${editUserId}`, { responseType: 'json' ,
        headers: {
            'Authorization': `Bearer ${isTokenExits}`
          }
      });    
      if(get_all_User?.data?.data) {   
        const userD  = get_all_User?.data?.data;

        const hospitalOptions = userD.hospital;
        const hospitalIds = Object.keys(hospitalOptions).map(Number);

        setFormData({ ...formData, 'full_name': userD.full_name, 'user_email':userD.user_email, 'trust':userD.trust, 'role':userD.primary_role ,'hospital':hospitalIds,'speciality':userD.speciality ,'sub_speciality':userD.sub_speciality});
        // get according to id data 
        getAllHospitals(userD.trust);
...
// if user type trust admin and hospital admin 
    ...
    return (
      <>
            <Modal show={edituserShow} onHide={() => setEdituserShow(false)} centered className='bottomSlideModalMob FullScreenModalMob'>
